...
# Given
echo "STEP: Given — create an event whose end date is today"
cat > "$EVENT_REQUEST_BODY" <<EOF
{
  "title": "${EVENT_TITLE}",
  "description": "End boundary registration event",
  "startDate": "${TODAY}",
  "endDate": "${TODAY}",
  "location": "Boundary Hall End"
}
EOF
...
# When
echo "STEP: When — register attendee on the event end date"
cat > "$REQUEST_BODY_FILE" <<EOF
{
  "eventId": "${EVENT_ID}",
  "name": "End Date User",
  "email": "${ATTENDEE_EMAIL}",
  "phone": "+1-555-202-0602"
}
EOF
...
# Then
echo "STEP: Then — assert registration is allowed on the end date"
