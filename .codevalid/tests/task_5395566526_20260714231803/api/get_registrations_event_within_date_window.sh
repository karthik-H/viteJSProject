...
# Given
echo "STEP: Given — create an event currently within a broad active date window and one registration"
echo "PREREQ: create event whose date range includes today"
cat > "$EVENT_REQUEST_BODY" <<EOF
{"title":"${EVENT_TITLE}","description":"Active event for retrieval","startDate":"2020-01-01","endDate":"2099-12-31","location":"Active Center"}
EOF
...
echo "PREREQ: create registration for active event"
cat > "$REG_REQUEST_BODY" <<EOF
{"eventId":"${EVENT_ID}","name":"Active User","email":"${ACTIVE_EMAIL}","phone":"444-444-4444"}
EOF
echo "REQUEST_HEADERS: Content-Type: application/json"
...
[ "$REG_HTTP_CODE" = "201" ] || { echo "ASSERTION_FAILED: expected HTTP 201 got ${REG_HTTP_CODE} while creating registration for active event"; exit 1; }

# When
echo "STEP: When — retrieve registrations for the event within the active date window"
...
# Then
echo "STEP: Then — assert HTTP 200 and returned registration for the active event"
[ "$HTTP_CODE" = "200" ] || { echo "ASSERTION_FAILED: expected HTTP 200 got ${HTTP_CODE}"; exit 1; }
grep -F '"eventId":"'"$EVENT_ID"'"' "$RESPONSE_BODY" >/dev/null 2>&1 || { echo "ASSERTION_FAILED: expected response body to contain the active event id"; exit 1; }
grep -F '"email":"'"$ACTIVE_EMAIL"'"' "$RESPONSE_BODY" >/dev/null 2>&1 || { echo "ASSERTION_FAILED: expected response body to contain the active registration email"; exit 1; }
