...
EVENT_ID=""
EVENT_TITLE="Expired Historical Event ${CASE_SUFFIX}"
...
# Given
echo "STEP: Given — create an expired event without creating new registrations"
echo "PREREQ: create event whose date range is entirely in the past"
cat > "$EVENT_REQUEST_BODY" <<EOF
{"title":"${EVENT_TITLE}","description":"Expired event for historical retrieval","startDate":"2020-01-01","endDate":"2020-12-31","location":"Archive Hall"}
EOF
...
[ "$EVENT_HTTP_CODE" = "201" ] || { echo "ASSERTION_FAILED: expected HTTP 201 got ${EVENT_HTTP_CODE} while creating expired event"; exit 1; }
...
echo "PREREQ: no registration is created because POST /api/registrations correctly blocks expired events"

# When
echo "STEP: When — retrieve registrations for the event outside the active date window"
...
# Then
echo "STEP: Then — assert HTTP 200 and historical retrieval remains allowed outside registration window"
[ "$HTTP_CODE" = "200" ] || { echo "ASSERTION_FAILED: expected HTTP 200 got ${HTTP_CODE}"; exit 1; }
BODY_COMPACT="$(tr -d '[:space:]' < "$RESPONSE_BODY")"
[ "$BODY_COMPACT" = "[]" ] || { echo "ASSERTION_FAILED: expected empty array [] for expired event with no registrations got ${BODY_COMPACT}"; exit 1; }
