...
EVENT_STATUS="$(curl -sS -o "$EVENT_RESPONSE" -w '%{http_code}' -X POST "$BASE_URL/api/events" \
  ...
  --data "{\"title\":\"Boundary Start Event ${CASE_SUFFIX}\",\"description\":\"Start date boundary\",\"startDate\":\"${TODAY}\",\"endDate\":\"${TODAY}\",\"location\":\"Boundary Room\"}")"
[ "$EVENT_STATUS" = "201" ]
...
curl -sS -o "$REG_RESPONSE" -w '%{http_code}' -X POST "$BASE_URL/api/registrations" \
  -H 'Content-Type: application/json' \
  --data "{\"eventId\":\"${EVENT_ID}\",\"name\":\"Start Date User\",\"email\":\"${ATTENDEE_EMAIL}\",\"phone\":\"+1-555-202-0601\"}" > "$STATUS_FILE"

# Then
STATUS="$(cat "$STATUS_FILE")"
[ "$STATUS" = "201" ]
grep -F "\"eventId\":\"${EVENT_ID}\"" "$REG_RESPONSE" >/dev/null
grep -F '"name":"Start Date User"' "$REG_RESPONSE" >/dev/null
